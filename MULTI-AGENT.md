...
> **NOTE:** Agent names (Assistant, Homer, Edith) are **examples**. When copying this architecture, replace them with your own names. The `install.sh` script handles renaming automatically.
...
|-------|------|-------|-----------|--------------|
| **Assistant** | Coordinator | **Opus** | Many Sonnet subagents (search, analysis) | `@assistant_bot` |
| **Homer** | Coder | **Opus** | Per-skill model (varies) | `@homer_bot` |
| **Edith** | Inbox / Knowledge | **Sonnet** | -- | `@edith_bot` |

### Why these models?

- **Assistant** on Opus: coordinator needs deep reasoning for task routing, planning, multi-step workflows. Delegates bulk search/analysis to cheap Sonnet subagents.
...
│            GATEWAY (1 process)          │
│                                         │
│  token_1 → poll @assistant  → agent=assistant │
...
```json
{
  "agents": [
    {
      "name": "assistant",
      "bot_token_env": "TG_TOKEN_ASSISTANT",
      "workspace": "~/.claude-lab/assistant/.claude",
      "model": "opus",
      "role": "coordinator"
    },
    {
...
> **Example names** -- replace `assistant/`, `homer/`, `edith/` with your agent names.
...
~/.claude-lab/
│
├── assistant/                             # COORDINATOR (example name)
│   └── .claude/
...
```bash
curl -X POST http://localhost:8095/hooks/assistant \
...
Messages in topic 42 go to Assistant, topic 43 to Homer. Messages outside configured topics are ignored.
  ...
  "agents": [
    {
      ...
      "agent_names": ["assistant", "homer", "edith"],
      "topic_routing": {}
    },
    {
      "name": "edith",
...
curl -X POST "http://localhost:1933/api/v1/resources" \
  ...
  -H "X-OpenViking-User: assistant" \
  -H "Content-Type: application/json" \
  -d '{"temp_file_id":"<from step 1>","to":"viking://resources/assistant-sessions/2026-04-10","wait":true}'
...
> Replace `my-team` with your account name. Replace `assistant` with the searching agent's name.
...
| **Message bus** | Agent-to-agent delegation | Assistant sends task to Homer |
...
```bash
# Assistant delegates to Homer
msgbus send homer \
  '{"from":"assistant","body":"Build the API endpoint for /users","priority":"P1"}'
...
AGENTS: assistant (plan), homer (code)

STEPS:
1. Assistant creates plan (architecture, files, tests)
2. Assistant delegates implementation to Homer with plan
3. Homer codes, tests, commits, creates PR
4. Assistant reviews PR
5. Assistant confirms completion to operator
```
...
AGENTS: assistant (review), edith (research)

STEPS:
1. Assistant receives request, extracts topic
2. Assistant delegates to Edith with structured request
3. Edith researches, organizes findings
4. Edith pushes to OpenViking
5. Assistant presents brief to operator
```
