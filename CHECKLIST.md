...
> **NOTE:** `assistant` is an example name. Replace with your own agent name.

## 1. Create Workspace

```bash
AGENT_NAME="assistant"  # ← replace with your agent name
...
```json
{
  "agents": {
    "assistant": {
      "enabled": true,
      "telegram_bot_token_file": "~/.claude-lab/shared/secrets/telegram/bot-token-assistant",
...
```bash
sudo cat > /etc/systemd/system/assistant-gateway.service << 'EOF'
[Unit]
Description=ASSISTANT Telegram Gateway
After=network.target

[Service]
Type=simple
User=YOUR_USER
WorkingDirectory=/home/YOUR_USER/.claude-lab/assistant
...
sudo systemctl enable assistant-gateway
sudo systemctl start assistant-gateway
```
